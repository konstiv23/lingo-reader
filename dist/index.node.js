...
function saveResource(resource, resourceSaveDir) {
  const { id, base64Data, contentType } = resource;
  {
    ...
    const buffer = node_buffer.Buffer.from(base64Data, "base64");
...
function saveStylesheet(style, resourceSaveDir) {
  {
    ...
    node_fs.writeFileSync(filePath, style);
    return filePath;
  }
}
...
class Fb2File {
  constructor(fb2, resourceSaveDir = "./images") {
    ...
    if (!node_fs.existsSync(this.resourceSaveDir)) {
      node_fs.mkdirSync(this.resourceSaveDir, { recursive: true });
    }
  }
  getToc() {
