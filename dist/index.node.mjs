import { writeFileSync, existsSync, mkdirSync, unlink } from 'node:fs';
import { camelCase, parsexml } from '@lingo-reader/shared';
...
const mimeTypeToResourceExtension = {
  // image
  "image/jpeg": "jpg",
  "image/png": "png",
  "image/gif": "gif",
...
function parseDocumentInfo(documentInfoAST) {
  ...
  for (const key in documentInfoAST) {
    if (key === "children") {
      ...
      documentInfo[camelCase(key)] = getFirstXmlNodeText(documentInfoAST[key]);
    }
  }
  return documentInfo;
}
function parsePublishInfo(publishInfoAST) {
  ...
  for (const key in publishInfoAST) {
    ...
    publishInfo[camelCase(key)] = getFirstXmlNodeText(node);
  }
  return publishInfo;
}
...
export { HREF_PREFIX, ID_PREFIX, STYLESHEET_ID, initFb2File };
